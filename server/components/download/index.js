 ...
 * @param {[type]} options
 */
const avoidMessyFilename = (req, res, filename, options) => {
 ...
 * @return {null}
 */
const download = (req, res, next, options) => {
    let mimeType;

    // 避免乱码
    avoidMessyFilename(req, res, options.filename);
    ...
    // 根据文件名自动识别文件类型
        ...
        const reg = /\.([^.]*)$/;
        const matchs = options.filename.match(reg);
