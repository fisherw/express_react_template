...
var _ = require('lodash'),
    ...
    resProcessor = require('../../components/res-processor');
 ...
 */
module.exports = (uri, otherParams) => {
    return async (req, res, next) => {
        ...
        // 如果有用户信息，添加用户id
        var userId = _.get(req, 'rSession.user.userId');
        if (userId) {
        ...
        // 追加扩展参数
        if (otherParams) {
            _.extend(params, otherParams);
        }


        try {
            result = await proxy.apiProxyPromise(uri, params, req);
        } catch (err) {
